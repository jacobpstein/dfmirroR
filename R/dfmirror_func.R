...
simulate_dataframe <- function(input_df, num_obs = 1, columns_to_simulate = colnames(input_df), hide_cols = FALSE) {
  ...
  if (hide_cols) {
    colnames(simulated_df) <- paste0("v", seq_along(columns_to_simulate))
  } else {
  ...
  for (i in seq_along(columns_to_simulate)) {
    ...
    if (is.numeric(input_df[[col]])) {
      ...
      if (shapiro_test$p.value > 0.05 && length(unique(input_df[[col]])) > 1) {
        dist_fit <- fitdistrplus::fitdist(input_df[[col]], "norm")
        simulated_df[[i]] <- rnorm(num_obs, mean = dist_fit$estimate[1], sd = dist_fit$estimate[2])
        code <- paste0(code, "  v", i, " = rnorm(", num_obs, ", mean = ", dist_fit$estimate[1], ", sd = ", dist_fit$estimate[2], "),\n")
      } else {
        input_df[[col]][is.na(input_df[[col]])] <- mean(input_df[[col]], na.rm = TRUE)
        skew_value <- e1071::skewness(input_df[[col]])

        if (!is.na(skew_value) && abs(skew_value) > 1) {
          simulated_df[[i]] <- simulate_skewed_gamma(input_df[[col]], num_obs)
          code <- paste0(code, "  v", i, " = simulate_skewed_gamma(input_df[,", which(colnames(input_df) == col), "], ", num_obs, "),\n")
        } else {
          sampled_values <- unique(input_df[[col]])
          simulated_df[[i]] <- sample(sampled_values, num_obs, replace = TRUE)
          code <- paste0(code, "  v", i, " = sample(c(", paste(sampled_values, collapse = ", "), "), ", num_obs, ", replace = TRUE),\n")
        }
      }
    } else if (is.factor(input_df[[col]])) {
      simulated_df[[i]] <- factor(sample(levels(input_df[[col]]), num_obs, replace = TRUE))
      code <- paste0(code, "  v", i, " = factor(sample(c(", paste(levels(input_df[[col]]), collapse = ", "), "), ", num_obs, ", replace = TRUE)),\n")
    } else if (is.character(input_df[[col]])) {
      simulated_df[[i]] <- replicate(num_obs, paste(sample(letters, 10, replace = TRUE), collapse = ""))
      code <- paste0(code, "  v", i, " = replicate(", num_obs, ", paste(sample(letters, 10, replace = TRUE), collapse = '')),\n")
    }
  }
